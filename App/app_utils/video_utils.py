...
class VideoUtils:
    ...
    def AddSingleAnnotation(self, img, bb_top: int, bb_left: int, bb_bottom: int, bb_right: int, category, object_id, thickness) -> None:
        ...
        fontScale = 0.5
        thickness = 2
        size_txt = cv2.getTextSize(text, fontFace, fontScale, thickness)
        _x11 = left
        _y11 = top
        _x12 = _x11 + max(size_txt[0][0], right-left)
        _y12 = _y11 + size_txt[0][1] + 15
        cv2.rectangle(img, (_x11, _y11), (_x12, _y12), color, cv2.FILLED)
        cv2.putText(img, size_txt, (_x11, (_y11+15)), fontFace, fontScale, (255, 255, 255), thickness)
    ...
    def AnnotateVideo(self, output_dir, orig_video, output_video, df_ann, start_time_sec=0, duration_sec=None, save_images=False) -> None:
        ...
        if video_in.isOpened():
            ...
            start_count, end_count = VideoUtils.GetStartEndCount(fps, total_frames, start_time_sec, duration_sec)
