...
class Evaluate:
    ...
    def GetMaxCorrelation(bb_gt, pred_bboxes, frame):
        ...
        for idx, row in pred_bboxes.iterrows():
            bb = [row["bb_left"], row["bb_top"],
                  row["bb_right"], row["bb_bottom"]]
            gt = Bbox(int(bb_gt[0]), int(bb_gt[1]),
                      int(bb_gt[2]), int(bb_gt[3]))
            pred = Bbox(int(float(bb[0])), int(float(bb[1])), int(
                float(bb[2])), int(float(bb[3])))
            iou = gt.IOU(pred)
    ...
    def AddIouToFrame(img, df_info):
        for index, row in df_info.iterrows():
            ...
            color = (0, 255, 255)
            cv2.putText(img, text, (bb.left + 10, bb.bottom - 10),
                        0, 0.5, color, 1)
        return img
    ...
    def AddIouToVideo(output_dir, orig_v_full_path, df_info):
        ...
        if video_in.isOpened():
            ...
            full_filename = os.path.join(
                output_dir, 'iou_' + os.path.basename(orig_v_full_path))
